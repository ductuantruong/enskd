...
from wespeaker.models.dkd_loss import DKD
...
def train(config='conf/config.yaml', **kwargs):
    ...
    teacher_model = get_speaker_model(configs['teacher_model'])(**configs['teacher_model_args'])
    teacher_model.eval()
    ...
    ddp_teacher_model = torch.nn.parallel.DataParallel(teacher_model, device_ids=[gpu])
    ddp_teacher_model.eval()
    device = torch.device("cuda")

    criterion = getattr(torch.nn, configs['loss'])(**configs['loss_args'])
    if configs['aux_loss'] is not None:
        aux_criterion = DKD(warmup=configs['aux_loss_args']['warmup_epoch'], gamma=configs['aux_loss_args']['gamma'])
    else:
        aux_criterion = None
    ...
    if rank == 0:
        ...
        logger.info("Aux loss criterion is: " + configs['aux_loss'])
    ...
    if rank == 0:
        ...
        header = ['Epoch', 'Batch', 'Lr', 'Margin', 'Loss', 'Aux Loss', "Acc"]
